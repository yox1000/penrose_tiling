...
# Adjusted place_tnr_tile function for closing at vertex 2
def place_tnr_tile():
    global current_vertex_index, vertices_table
    vertices_indices = []
    
    # Define key vertices 1, 2, and 10 as per orientation rules
    p0 = vertices_table[1]  # Starting at vertex 1
    p1 = vertices_table[2]  # Second-lowest vertex is 2
    p2 = vertices_table[10]  # Third vertex in clockwise order
    # In place_tnr_tile function, update the origin parameter of rotate_point as follows
    p3 = rotate_point(p1, angle_144, origin=np.array(p2[:2]))  # Correct origin argument


    # Ensure the tile returns to p1 (vertex 2) to complete the loop correctly
    vertices = [p0, p1, p2, p3]  # Reorder to close at p1
    for vertex in vertices:
        vertex_index = find_vertex(vertex)
        if vertex_index is None:
